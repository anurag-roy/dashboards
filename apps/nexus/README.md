...
This document explains how the `apps/nexus` app is structured, what each route renders, and which components/patterns are used so another agent can onboard quickly.
...
- Marketing metadata and canonical URLs live in `app/siteConfig.ts` (product name: **Nexus AI**).
