...
Dashboards Hub is the product index for the four demo dashboards in this monorepo. It runs on port `3000` and serves the future `dashboards.anuragroy.dev` surface.

Use this README as the first context file before editing the hub page.

## Purpose

- Collection landing page that introduces Orbit, Audit, Pulse, and Nova as four distinct dashboard products.
- Main workflow: scan the product collection, understand each dashboard's focus, then open a relevant demo.
...
- Header links back to the hub root and into the product collection.
- Hero copy introduces the four-product collection without personal or portfolio positioning.
...
- Each product chapter uses the app's logo pair, theme class, focused positioning, and a screenshot captured from the actual dashboard.
...
- Keep rendered copy focused on the products and their workflows. Save framework, hosting, and port details for docs.
