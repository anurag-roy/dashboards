...
This document explains how the `apps/pulse` app is structured, what each route renders, and which components/patterns are used so another agent can onboard quickly.
...
- Product metadata lives in `app/siteConfig.ts` (product name: **Pulse**).
