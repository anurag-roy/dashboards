...
const sortedTickets = tickets.sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime());

// Write to file
fs.writeFileSync(
  path.join(__dirname, 'tickets.ts'),
  `import { Ticket } from "./schema";\n\nexport const tickets: Ticket[] = ${JSON.stringify(sortedTickets, null, 2)};`
);
