...
const regions = ['Zürich', 'Geneva', 'Basel', 'Bern', 'Lugano', 'Lausanne', 'Lucerne', 'Winterthur'] as const;
...
const agents = Array.from({ length: 102 }, () => {
  ...
  return {
    ...
    end_date: faker.datatype.boolean({ probability: 0.22 })
      ? faker.date.between({ from: startDate, to: '2026-04-28T00:00:00Z' }).toISOString()
      : null,
    account: faker.helpers.arrayElement(accounts).label,
    number: faker.phone.number({ style: 'international' }),
    email: `${fullName.charAt(0).toLowerCase()}${lastName
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-z0-9]/g, '')}@pulse.com`,
    registered: faker.datatype.boolean({ probability: 0.82 }),
    minutes_called: Math.floor(minutes_booked * faker.number.float({ min: 0.4, max: 0.99 })),
    minutes_booked,
    ticket_generation: faker.datatype.boolean({ probability: 0.8 }),
    status: faker.helpers.arrayElement(['online', 'away', 'offline']),
    region: faker.helpers.arrayElement([...regions]),
    stability: Math.round(faker.number.float({ min: 42, max: 99, fractionDigits: 0 })),
    ticket_count: faker.number.int({ min: 0, max: 980 }),
    rating: Math.round(faker.number.float({ min: 2.4, max: 5.0, fractionDigits: 1 }) * 10) / 10,
  };
});
