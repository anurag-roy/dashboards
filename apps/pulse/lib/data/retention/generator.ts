...
import { CohortRetentionData, CohortSummary, CohortsAggregate, WeekData } from './schema';
...
const generateCohortData = (): CohortRetentionData => {
  ...
  for (let cohortIndex = 0; cohortIndex < cohortsToGenerate; cohortIndex++) {
    const cohortStartDate = new Date(startDate.getTime() + cohortIndex * 7 * 24 * 60 * 60 * 1000);
    const cohortEndDate = new Date(cohortStartDate.getTime() + 7 * 24 * 60 * 60 * 1000);
    ...
    for (let week = 0; week < weeksToTrack; week++) {
      ...
      const retention_rate = week === 0 ? 100 : generateDecimal(baseRetention - 5, baseRetention + 5, 1);
...
function generateCohortsAggregate(cohorts: CohortRetentionData): CohortsAggregate {
  ...
  cohortEntries.forEach(([, cohort]) => {
    ...
    // Activity metrics
    summary.aggregateMetrics.activity.totalTicketsCreated += cohort.summary.activity.total_tickets_created;
    summary.aggregateMetrics.activity.totalTicketsResolved += cohort.summary.activity.total_tickets_resolved;
    summary.aggregateMetrics.activity.avgCallsMade += cohort.summary.activity.total_calls_made;
    summary.aggregateMetrics.activity.avgChatSessions += cohort.summary.activity.total_chat_sessions;
    summary.aggregateMetrics.activity.avgEmailInteractions += cohort.summary.activity.total_email_interactions;

    // Satisfaction metrics
    summary.aggregateMetrics.satisfaction.avgCsatScore += cohort.summary.satisfaction.avg_csat_score;
    summary.aggregateMetrics.satisfaction.avgNpsScore += cohort.summary.satisfaction.avg_nps_score;
    summary.aggregateMetrics.satisfaction.totalFeedbackResponses +=
      cohort.summary.satisfaction.total_satisfaction_responses;

    // Performance metrics
    summary.aggregateMetrics.performance.avgResponseTimeMinutes += cohort.summary.performance.avg_response_time_minutes;
    summary.aggregateMetrics.performance.avgHandlingTimeMinutes += cohort.summary.performance.avg_handling_time_minutes;
    ...
    summary.aggregateMetrics.performance.avgEscalationRate += cohort.summary.performance.avg_escalation_rate;
    ...
    cohort.weeks.forEach((week, index) => {
      ...
      summary.aggregateMetrics.retention.averageRetentionByWeek[index] += week.percentage;
    });

    // Process top issues
    cohort.summary.top_issues.forEach((issue) => {
      const existingIssue = summary.commonIssues.find((i) => i.category === issue.category);
      if (existingIssue) {
        existingIssue.totalCount += issue.count;
        existingIssue.avgResolutionRate = roundToDecimal((existingIssue.avgResolutionRate + issue.resolution_rate) / 2);
      } else {
  ...
  activity.avgTicketsCreated = roundToDecimal(activity.totalTicketsCreated / totalCohorts);
  activity.avgTicketsResolved = roundToDecimal(activity.totalTicketsResolved / totalCohorts);
  activity.ticketResolutionRate = roundToDecimal(activity.totalTicketsResolved / activity.totalTicketsCreated);
  activity.avgCallsMade = roundToDecimal(activity.avgCallsMade / totalCohorts);
  activity.avgChatSessions = roundToDecimal(activity.avgChatSessions / totalCohorts);
  activity.avgEmailInteractions = roundToDecimal(activity.avgEmailInteractions / totalCohorts);

  const satisfaction = summary.aggregateMetrics.satisfaction;
  satisfaction.avgCsatScore = roundToDecimal(satisfaction.avgCsatScore / totalCohorts);
  satisfaction.avgNpsScore = roundToDecimal(satisfaction.avgNpsScore / totalCohorts);
  satisfaction.positiveFeedbackRate =
    satisfaction.totalFeedbackResponses > 0
      ? roundToDecimal(
          cohortEntries.reduce((sum, [, cohort]) => sum + cohort.summary.satisfaction.total_positive_feedback, 0) /
  ...
  satisfaction.negativeFeedbackRate =
    satisfaction.totalFeedbackResponses > 0
      ? roundToDecimal(
          cohortEntries.reduce((sum, [, cohort]) => sum + cohort.summary.satisfaction.total_negative_feedback, 0) /
            satisfaction.totalFeedbackResponses
        )
      : 0;

  const performance = summary.aggregateMetrics.performance;
  performance.avgResponseTimeMinutes = roundToDecimal(performance.avgResponseTimeMinutes / totalCohorts);
  performance.avgHandlingTimeMinutes = roundToDecimal(performance.avgHandlingTimeMinutes / totalCohorts);
  performance.avgFirstContactResolutionRate = roundToDecimal(performance.avgFirstContactResolutionRate / totalCohorts);
  performance.avgEscalationRate = roundToDecimal(performance.avgEscalationRate / totalCohorts);

  // Calculate average retention by week with rounding
  summary.aggregateMetrics.retention.averageRetentionByWeek =
    summary.aggregateMetrics.retention.averageRetentionByWeek.map((total) => roundToDecimal(total / totalCohorts));
  summary.aggregateMetrics.retention.overallRetentionRate = roundToDecimal(
    summary.aggregateMetrics.retention.averageRetentionByWeek.reduce((sum, rate) => sum + rate, 0) /
