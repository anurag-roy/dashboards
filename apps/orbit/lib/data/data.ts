...
const shiftUsageYear = (value: string) => {
  const parsedValue = /^(\d{2}\/\d{2}\/)(\d{4})(\s\d{2}:\d{2})$/.exec(value);
  if (!parsedValue) return value;

  const [, dayMonth, year, time] = parsedValue;
  const shiftedYear = Math.min(Number(year) + 4, 2026);
  return `${dayMonth}${shiftedYear}${time}`;
};

export const usage: Usage[] = usageSeed.map((entry) => ({
  ...entry,
  lastEdited: shiftUsageYear(entry.lastEdited),
}));
