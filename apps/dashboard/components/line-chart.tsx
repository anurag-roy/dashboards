  ...
  return (
    <li
      className={cn(
        ...
        hasOnValueChange ? 'bg-transpaent cursor-pointer hover:bg-muted' : 'cursor-default'
      )}
      onClick={(e) => {
        e.stopPropagation();
        onClick?.(name, color);
...
const OverviewChartTooltip = ({ active, payload, categoryColors, valueFormatter }: ChartTooltipProps) => {
  if (active && payload) {
    ...
    return (
      <div className={cn('rounded-2xl border border-border bg-popover text-sm text-popover-foreground shadow-lg')}>
        <div className='flex items-start justify-between gap-2 border-b border-border px-3 py-2.5'>
          <p className={cn('font-medium text-foreground')}>{title}</p>
          {evolution !== undefined && (
            <InsightBadge variant={getBadgeType(evolution)}>{percentageFormatter(evolution)}</InsightBadge>
          )}
        </div>
        <div className={cn('space-y-1 px-3 py-2.5')}>
  ...
  return (
    <div ref={ref} className={cn('h-80 w-full', className)} {...other}>
      <ResponsiveContainer>
        ...
        >
          {showGridLines ? (
            <CartesianGrid className={cn('stroke-border stroke-1')} horizontal={true} vertical={false} />
          ) : null}
          <XAxis
            ...
            ticks={(() => {
              ...
              if (!startEndOnly || !firstRow || !lastRow) return undefined;
              return [
                firstRow[index as keyof typeof firstRow] as string | number,
                lastRow[index as keyof typeof lastRow] as string | number,
              ];
            })()}
            fill=''
            stroke=''
