...
function Card({ className, size = 'default', ...props }: React.ComponentProps<'div'> & { size?: 'default' | 'sm' }) {
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card flex flex-col gap-6 overflow-hidden rounded-2xl bg-card py-6 text-sm text-card-foreground shadow-sm ring-1 ring-foreground/6 has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4 dark:shadow-none dark:ring-foreground/10 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl',
        className
      )}
      {...props}
    />
